...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logging.getLogger(__name__).info(
        'Removed %d rows with missing country',
        before - after
    )
...
def write_data(df: pd.DataFrame, path: Path) -> None:
    ...
    logging.getLogger(__name__).info("Data written to Parquet with partition on 'country'")
...
def main():
    ...
    try:
        logger.info(f'Data pipeline started')
        # extract
        users_file = base_dir / 'data/users_original.csv'
        users = load_data(users_file)
        logger.info(f'Users dataset loaded: %d rows:', len(users))
        orders_file = base_dir / 'data/orders_original.csv'
        orders = load_data(orders_file)
        logger.info(f'Orders dataset loaded: %d rows:', len(orders))

        # transform
        orders_with_users = merge_data(orders, users)
        logger.info(f'Merged dataset created: %d rows:', len(orders_with_users))
        cleaned = clean_data(orders_with_users)
        logger.info(f'Cleaned dataset created: %d rows:', len(cleaned))

        # aggregate
        analyzed = analyze_data(cleaned)
        logger.info(f'Aggregated dataset created: %d rows:', len(analyzed))
